# 9. Integrating AI into OpenUSD Workflows
...
## 9.1 Preparing to Chat with LLM's
...
9.1.1 Installing the OpenAI SDK
...
Installing Openai with Google Collab                                                      .
...
Series
	...
	Most advanced, well-regarded for their versatility in handling a wide range of tasks
	...
	High flexibility, multi-language ability
	...
	Relatively small, efficient and robust
...
client = OpenAI(api_key = <"your_openai_api_key_here">)    #A

#A Replace with your OpenAI API key
...
prompt = "What is OpenUSD?"    #A

completion = client.chat.completions.create(    #B

        model="gpt-4o-mini",    #C

        messages=[    #D

        {"role": "user","content": prompt}    #E

        ]

)

#A Defines the user’s input question as a string

#B Calls the API to generate a response from the model

#C Specifies the model to use (GPT-4o-mini)

#D Defines the conversation context

#E The user’s prompt is sent as a message with role "user"
...
print(completion.choices[0].message.content)    #A

#A Extracts and prints the AI-generated response
...
client = OpenAI(    #A
    ...
    base_url = "https://integrate.api.nvidia.com/v1",    #B  

    api_key = <"your_nvidia_api_key_here">    #C

    )

#A Initializes the OpenAI client object to interact with the API.

#B Sets the base URL for API requests, specifying the endpoint for NVIDIA’s API.

#C Provides the API key required for authentication with the NVIDIA API.
...
prompt = "What are the main features of OpenUSD?"    #A
...
completion = client.chat.completions.create(    #B
    ...
    model="meta/llama-3.1-405b-instruct",    #C
...
#C Specifies the model to use (meta/llama-3.1-405b-instruct)

#D Defines the conversation context

#E The user’s prompt is sent as a message with role "user"

#F Extracts and prints the AI-generated response
...
9.2.2 Writing Code for OpenUSD
...
system_message = "You are a helper for coding using Python for OpenUSD."    #A
...
completion = client.chat.completions.create(
        ...
        {"role": "system","content": system_message},    #B
...
#A Defines the system message that instructs the model to focus on assisting with Python coding for OpenUSD.

#B Adds the system message to set the model's behavior.
...
code = code_match.group(1) if code_match else content    #C 
...
#A Imports the 're' module for working with regular expressions in Python.

#B Searches for code enclosed in triple backticks (optionally labeled with 'python') in the 'response' string using a regular expression.

#C If a match is found, extracts the code between the backticks; otherwise, uses the original 'content'.

#D Prints the extracted code or the original content if no code is found.
...
{ref}`figure-10-1` Printed code results from calling OpenAI's API. By executing the code, you can get a basic_geometries.usda file saved under the working directory.
...
9.2.3 Talking with LLMs about USDA Files

Thanks to the human-readable format of .usda files, LLMs can parse and understand them, facilitating tasks like generating and interpreting commands, suggesting modifications, and providing content-based insights. Let's try it out using an example .usda file, shown in {ref}`figure-10-2`, named 'Desktop.usda', and located within the 'Ch09' directory on our GitHub, accessible here: https://github.com/linfanz/OpenUSDInAction. Ensure you have set your working directory to 'Ch09' so that the .usda file is accessible for the following scripts. 

  



{ref}`figure-10-2` A rendering of the Desktop.usda file. The stage is composed of a desk, a fan, a monitor, and more.
...
usd_file_path = <your path to Desktop.usda file ex: "./Desktop.usda">    #A





with open(usd_file_path, 'r') as file:    #B

    usda_content = file.read()    #C

#A Define the file path to the .usda file 

#B Open the file in read ('r') mode 

#C Read the entire content of the .usda file into a string
...
messages = [  #A
        ...
        {"role": "system", "content": "You are an expert on USDA files and OpenUSD stages."},  #B

        {"role": "user", "content": f"The following is the content of a USDA file:\n\n{usda_content}"}  #C

]

#A Define a list of message dictionaries to structure the conversation with the LLM

#B System message setting the LLM’s behavior and expertise

#C Inserts the content of the USDA file into the prompt using an f-string
...
messages.append({"role": "user", "content": prompt})    #A

#A Append the user message to the existing messages list for the LLM 

By calling the chat completion API as before, we can get the description of the .usda stage without even opening it:
...
transform_data = "\n\n".join(transform_matches) if transform_matches else "No transform data found."    #C
...
#A Import the regular expressions module

#B Search for all occurrences of text enclosed within triple backticks (optionally labelled 'plaintext')

#C Join extracted transform data into a single formatted string

#D Print only the extracted transform-related information
...
{ref}`figure-10-3` The result of using regex to extract the Fan's transform data from the full text of the LLM's initial response.
...
9.2.4 Editing the Stage with LLMs
...
usda_text = usda_match .group(1) if usda_match else content    #B

#A Search for content inside triple backticks (optionally labeled 'usda') in the response

#B Extract the matched content inside the backticks or set 'content' if no match is found
...
with open("./updated_desktop.usda","w") as file:
    ...
    file.write(usda_text)    #A

#A Open the file "updated_desktop.usda" in write mode ('w'), creating it if it doesn't exist

#B Write the content of 'usda_text' to the file
...
{ref}`figure-10-4` The updated Desktop.usda file. The fan and keyboard are rotated by 30 degrees around the up-axis. The monitor is moved to the right by 0.15 units. The intensity of the overhead light is decreased to 3000. 
...
9.3.1 Creating and Applying a Diffuse Texture
...
response = client.images.generate(    #A
...
#A Calls the OpenAI API to generate an image and stores the response

#B Ratio appropriate for a monitor screen

#C Opt for 'standard' quality

#D Set the number of images to generate
...
image_url = response.data[0].url    #A

image_data = requests.get(image_url).content    #B

with open('./Assets/textures/new_screen_DIFFUSE.png', 'wb') as file:    #C 

        file.write(image_data)    #D 

#A Extracts the URL of the first generated image from the API response

#B Downloads the image content from the retrieved URL

#C Opens a file named 'new_screen_DIFFUSE.png' in binary write mode

#D Writes the downloaded image data to the file

To apply our new image texture to the monitor in Desktop.usda, we have a choice regarding the scope of the change. Remembering the way referencing works in OpenUSD (see Figure 2.6 from Chapter 2), it allows us to modify either the original Monitor.usd asset (affecting all references) or just the instance within Desktop.usda (a non-destructive approach). Opting for the latter to preserve the original asset, let's open the Desktop.usda.
...
stage= Usd.Stage.Open(<your path to desktop.usda ex: "./Desktop.usda">)    #A

#A Open the Desktop.usda using your file path or a relative path
...
attr_path = "/World/Desk/Monitor/materials/Screen/preview_Image_Texture.inputs:file" #A

stage.GetAttributeAtPath(attr_path).Set("./Assets/textures/new_screen_DIFFUSE.png")  #B
...
#A Defines the USD attribute path to modify

#B Gets the attribute at the specified path and sets its value to the new image asset path
...
{ref}`figure-10-5` The left image displays the original monitor screen which remains unchanged in the Monitor.usd file, while the right image shows the Monitor.usd file as it will appear on the Desktop.usda stage with the AI-generated image texture applied. 
...
We can take it step further by generating multiple texture maps that can be used to create a material. If you need to revise the relationship between textures and materials, refer back to Chapter 3.

9.3.2 Generating Multiple Textures for Materials
...
{ref}`figure-10-6` The seamless pattern on the Floor.usd. Left is the individual image texture, Right is the texture repeated four times in a 2x2 grid, showing how it repeats perfectly.
...
API_URL = "https://api-inference.huggingface.co/pipeline/text-to-image/gokaygokay/Flux-Seamless-Texture-LoRA"    #B

headers = {    #C

    "Authorization": "Bearer <YOUR_HUGGINGFACE_API_TOKEN>"    #D
...
payload = {    #E
    ...
    "inputs": "smlstxtr, seamless, tileable, photorealisitic marble stone floor texture with realistic, even lighting, natural color variation, and no baked-in shadows. The texture should be a flat, top-down view of a 10x10 meter floor, seamless texture", #F

    "parameters": {

        "guidance_scale": 7.5,    #G

        "num_inference_steps": 30,    #H
...
response = requests.post(API_URL, headers=headers, json=payload)    #I





if response.status_code == 200 and response.headers["content-type"].startswith("image/"):    #J

    with open("./Assets/textures/seamless_texture_SD3-5.png", "wb") as f:    #K

        f.write(response.content)    #L
    ...
    print(f"Unexpected response: {response.status_code}, content type: {response.headers.get('content-type')}")    #M

#A Imports the 'requests' library to handle HTTP requests.

#B Sets the Hugging Face API endpoint for the Flux texture generation model.

#C Defines the HTTP headers, including the required Hugging Face API token.

#D Replace with your actual token.

#E Constructs the payload with the text prompt and generation parameters.

#F The generation prompt containing the trigger phrase for the LoRA, 'smlstxtr'

#G Controls prompt adherence; higher values stick more closely to the prompt.

#H The number of diffusion steps (quality vs speed tradeoff).

#I Send a POST request to the Hugging Face API with the payload.

#J If successful, saves the binary image content to a PNG file.

#K Opens a file in binary write mode, and saves it in the Assets/textures directory of "Ch09"

#L Writes the image data.

#M If the request fails, print the error code and message.
...
img = Image.open("./Assets/textures/marble_texture_DIFFUSE.png")    #B
...
grid.paste(img, (w, h))    #E





grid.save("marble_texture_2x2_grid.png")    #F

grid.show()

#A Imports the Python Imaging Library (Pillow) module for image processing

#B Loads the texture image from disk

#C Gets the original image's width and height

#D Creates a blank 2x2 grid, doubling the width, and height of the original to hold four copies of the image

#E Paste the texture 4 times to make a 2x2 grid (top left, top right, bottom left, bottom right)

#F Save the 2x2 grid as a new image

#G Opens the image in your default viewer so you can visually inspect for tiling seams
...
gray_img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)    #B





height_map = gray_img.astype('float32') / 255.0    #C
...
sobel_y = cv2.Sobel(height_map, cv2.CV_32F, 0, 1, ksize=5)    #D
...
normal_z = np.ones_like(height_map)    #F
...
norm = np.sqrt(normal_x**2 + normal_y**2 + normal_z**2)    #G

normal_map = np.stack([    #H
...
Image.fromarray(normal_map).save(out_path)    #J

#A Imports OpenCV for image processing, NumPy for numerical operations, PIL to save the normal map image

#B Load diffuse texture and convert to grayscale height map

#C Normalize height values to [0, 1] by converting pixel values from 0–255 to 0.0–1.0

#D Compute gradients using Sobel filter

#E Strength controls how pronounced the normals are. Low for smooth, high for pronounced

#F X, and Y components of normal multiplied by strength (Z component is fixed to 1 to point "outward")

#G Magnitude of each normal vector

#H Stack and remap XYZ vectors to RGB space (0–255)

#I Casts the data back to image format

#J Saves the RGB normal map as a PNG file to the out_path
...
img = Image.open(input_path).convert("L")    #A





# Normalize pixel values to range 0.3 to 0.7 (subtle roughness variation)

arr = np.array(img) / 255.0    #B

scaled_arr = 0.3 + (arr * 0.4)    #C





scaled_img = Image.fromarray((scaled_arr * 255).astype(np.uint8))    #D
...
scaled_img.save(output_path)    #E

#A Load the texture and convert it to grayscale using luminance

#B Normalize to [0.0, 1.0] float range

#C Remap to [0.3, 0.7] for subtle roughness values

#D Convert back to image from array using scale [0, 255] and converting to 8-bit

#E Save the Roughness map to the output_path
...
def find_texture_attributes(stage):    #A
    ...
    for prim in stage.Traverse():
        ...
        for attr in prim.GetAttributes():
            ...
            if "inputs:" in attr.GetName():    #C

                val = attr.Get()

                if isinstance(val, Sdf.AssetPath):

                    texture_attrs.append((attr, val.path))  #D

    return texture_attrs

#A Define a function that takes a Usd.Stage object

#B Define a list to collect found texture attributes

#C Traverse all prims on the stage. For each prim found, filter attributes that are likely shader inputs

#D Get the value of the attribute, check if it's a file reference, and store the attribute and its path

#E Return the list of found texture attribute/path pairs as 'texture_attr'
...
textures = find_texture_attributes(stage)    #A
...
new_normal_texture = "./Assets/textures/marble_texture_NORMAL.png"    #B





new_textures = [new_diffuse_texture, new_roughness_texture, new_normal_texture]    #C
...
if len(textures) == 3:    #D
    ...
    for i, (texture_attr, _) in enumerate(textures):   #E

        texture_attr.Set(Sdf.AssetPath(new_textures[i]))  #F
...
#A Open the Floor.usda using your file path or a relative path, and call the helper function

#B Define paths to the new textures (replace with your actual texture file paths if necessary)

#C Store the new texture paths as a list in the order they should be applied

#D Check that exactly three texture attributes were found before proceeding

#E Iterate over the found texture attributes

#F Replace the attribute’s value with a new asset path
...
{ref}`figure-10-7` The AI generated, seamless marble texture on the Floor.usda. Left are the individual image textures, right is the texture repeated four times in a 2x2 grid.
...
9.4 Managing 3D Assets with Vector Databases
...
!pip install -U pymilvus    #A

#A Install pymilvus - Note: the '!' is for running in Colab cells

Note You will likely get some dependency resolver errors when running this, but provided you get the 'Successfully Installed' message, there should be no issues.
...
milvus_client = MilvusClient("./milvus_demo.db")    #A
...
# A Initialize a Milvus client named "milvus_demo.db" in your working directory

#B Create and define the collection name and dimension
...
link = "https://github.com/yizhouzhao/OpenUSDInAction/tree/main/Ch09/NVIDIA_Assets/"    #A

asset_names = ["RackSmallEmpty_A1.usd", "RackLongEmpty_A1.usd", "WoodenCrate_A1.usd", "MetalFencing_A2.usd"]    #B

asset_links = [link + asset_name for asset_name in asset_names]    #C

#A Define the base URL of the asset folder

#B Create a list of file names for the assets

#C Produce a list of asset urls 
...
#A Define a list of natural language descriptions of each asset
...
def get_openai_embedding(text):
    ...
    response = client.embeddings.create(
        ...
        model="text-embedding-3-small"    #B

    )

    return response.data[0].embedding    #C





vectors = [get_openai_embedding(doc) for doc in docs]    #D

# A Send the input text to the OpenAI API for embedding

# B Specify the embedding model to use

# C Extract and return the embedding vector from the API response

#D Generate an embedding for each asset description and store in a list
...
data = [ {"id": i, "vector": vectors[i], "text": docs[i], "link": asset_links[i]} for i in range(len(vectors)) ]    #A





milvus_client.insert(collection_name="demo_collection", data=data)    #B

# A Iterate over all assets to construct dictionaries for each asset

# B Insert the compiled data into the Milvus collection
...
search_text1 = "An object that separates different areas"    #A

embedding1 = get_openai_embedding(search_text1)    #B
...
# A Defines a natural language search query

#B Converts the query into a semantic vector using OpenAI's embedding model via the previously defined function
...
result = milvus_client.search(
    ...
    collection_name="demo_collection",    #A

    data=[embedding1, embedding2],    #B

    limit=1,    #C

    output_fields=["text", "link"]    #D

)

print(result)    #E

# A Name of the collection to search in

#B List of query vectors generated from user text

#C Return only the top match for each query

#D Include original asset description and download URL in results
...
Unlocking the Future of AI for OpenUSD Workflows
