...
<!-- +++ {"part": "abstract"}
This section expands on previous skills—creating stages, adding materials and lighting, and using cameras—by introducing animation through the fourth dimension: time. To animate a scene in OpenUSD, a timeline must be established using tools like UsdStage and UsdTimeCode, which enable time-based transformations of objects, lights, and cameras. By assigning property values at specific times (keyframes), users can define motion and then use interpolation to create smooth transitions.
+++ -->
