...
<!-- +++ {"part": "abstract"}
While earlier chapters introduced basic object transformations, this section delves into more complex and practical applications, such as aligning components in engineering models, enabling interaction in digital twins, or spatial awareness in autonomous systems. You'll explore how transformation order affects outcomes, build a solid foundation in relevant math and geometry, and learn to programmatically control translation, rotation, and scaling—especially in relation to other objects. Key techniques like collision detection are also introduced, enabling physical interactions within the scene. These skills are essential for accurate scene composition and prepare you for more advanced topics ahead.
+++ -->


Let’s focus on the essential skills needed to dynamically control the elements within your scene. Up until now we’ve been using a simple approach to transforming objects, in order to introduce the basic concepts. However, there are more complex ways of working with Xforms that can prove both useful and efficient in many types of applications for OpenUSD.
...
{ref}`figure-6-1` shows how altering the transform order on each cube results in a different final position. Notice how the initial rotation of ‘cube1’ altered the angle of its y axis, so that when the translation was applied, the cube moved 10 units in a different direction than ‘cube2’.
