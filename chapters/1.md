...
{ref}`code-1` below, shows an example of a .usda file which explicitly describes a 3D cube, capturing its essential attributes, including its bounding extent, facial structure, vertex points, transformation data, and color information.
...
```{figure} ./images/1/image8.png
:label: figure-1-8
:alt: OpenUSD Cooperation
:align: center 
...
If we were to zoom into the USD stage shown in [Figure 4](#figure-4), we can illustrate how the layering system could work in the context of an animated scene. {ref}`figure-1-6` shows the way data can flow from lower layers through higher layers, passing through higher layers where it might be overridden, to ultimately reach the top layer that will be displayed on the actual stage.
